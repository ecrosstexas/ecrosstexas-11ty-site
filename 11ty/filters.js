...
module.exports = {
  readableDate: function (date, format) {
    // default to America/Chicagp Timezone
  ...
  webmentionCountByType: function (webmentions, url, ...types) {
    ...
    return String(
      webmentions.filter(isUrlMatch).filter(entry => types.includes(entry['wm-property']))
        .length
    );
  },
  ...
  excerpt: function (content) {
    ...
    const findExcerptEnd = content => {
      ...
      if (paragraphEnd < excerptMinimumLength) {
        return (
          paragraphEnd + findExcerptEnd(content.substring(paragraphEnd), paragraphEnd)
        );
      }
